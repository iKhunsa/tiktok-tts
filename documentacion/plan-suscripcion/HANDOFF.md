...
| 04 Backend app | `04-agente-backend-app-electron.md` | 🟡 dominio hecho, gates pendientes (2026-09-08) | `features/auth/` ✅, gating pendiente, `04-contrato-sesion.md` ✅, `04-features-pro.md` ✅ | Rama `feat/suscripciones-auth` (commits 0839fc2, 0e361a9). `features/auth/` completo: config no-op, session store, cliente, refresh, contrato `entitlements`, rutas `/api/auth/*`, 2 tools MCP + state provider, flag `subscriptionsEnabled`. `test/auth-domain.test.js` 6 casos, `npm test` 76/76, `check-mcp` ok. **Falta**: cablear los 7 gates en `features/{bot,sonido,movil,clips,mcp,canales,promo}/`, mapear `auth.*` en `glitchtip.js`/`aptabase.js`. Split free/Pro confirmado (overlays = free). |
| 05 Frontend app | `05-agente-frontend-app-electron.md` | pendiente | vista Cuenta, `sesion.js`, candados, i18n | — |
...
| **D — Contrato de sesión definido** | `04-contrato-sesion.md` escrito; `features/auth/` responde `bus.on('auth:get')` con esa forma. | ✅ **CERRADO** 2026-09-08 — `04-contrato-sesion.md` + `features/auth/` responde `auth:get` con la forma documentada (test lo verifica). El Agente 05 puede arrancar. | 05 (todo) | — |
...
| Tras agente 05, tarea 13 | ¿La vista Cuenta es `pinned` en el sidebar u oculta por defecto? | pendiente |
...
- 2026-09-08 — orquestador — **Fase 3 planning.** Inventario de features (subagente). Split free/Pro confirmado con el usuario. `04-features-pro.md` + `04-contrato-sesion.md` escritos. Siguiente: código de `features/auth/` en el repo `tiktok-tts` (rama nueva).
