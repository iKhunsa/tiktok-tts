...
| 04 Backend app | `04-agente-backend-app-electron.md` | planning hecho, código pendiente (2026-09-08) | `features/auth/`, gating, `04-contrato-sesion.md` ✅, `04-features-pro.md` ✅ | Inventario de features hecho. **Split free/Pro confirmado por el usuario**: Pro = `bot-musical`, `soundpad`, `panel-movil`, `clips`, `mcp-agente`, `multi-canal` (2+ canales de la misma plataforma), `sin-promos` (avisos de voz no suenan en Pro). Overlays = **free** (se rediseñan pronto). Falta: escribir `features/auth/` + cablear los 7 gates + wiring en `server.js` + tests. |
...
| **D — Contrato de sesión definido** | `04-contrato-sesion.md` escrito; `features/auth/` responde `bus.on('auth:get')` con esa forma. | 🟡 `04-contrato-sesion.md` escrito; falta que `features/auth/` exista y responda | 05 (todo) | — |
